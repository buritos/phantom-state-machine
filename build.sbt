...
lazy val fsm = project.in(file("."))
  .settings(name := "phantom-state-machine")
  .aggregate(proc, oo, fp)
  .settings(packagedArtifacts := Map.empty)

lazy val proc = project
  .settings(name := "phantom-state-machine-proc")

lazy val oo = project
  .settings(name := "phantom-state-machine-oo")
